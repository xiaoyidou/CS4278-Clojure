...
(defrecord Restaurant [name type price])

(def db (list (Restaurant. "McDonalds" ["fast food"] 1)
         (Restaurant. "Hattie B's Chicken - Midtown" ["american" "southern"] 2)
         (Restaurant. "Urban Cookhouse" ["american"] 2)
         (Restaurant. "The Row" ["southern"] 2)
         (Restaurant. "Nada" ["mexican"] 2)
         (Restaurant. "Peg Leg Porker" ["barbecue"] 2)
         (Restaurant. "Biscuit Love: Gulch" ["southern" "american"] 2)
         (Restaurant. "The Stillery" ["american" "pizza"] 2)
         (Restaurant. "Taco Mama" ["mexican"] 1)
         (Restaurant. "Martin's Bar-B-Que Joint" ["barbecue"] 2)
         (Restaurant. "The Grilled Cheeserie" ["sandwiches" "food trucks"] 1)
         (Restaurant. "Milk and Honey Nashville" ["american"] 2)
         (Restaurant. "Rotier's Restaurant" ["american" "burgers"] 1)
         (Restaurant. "Tavern" ["american"] 2)
         (Restaurant. "Boom Burgers" ["burgers"] -1)
         (Restaurant. "Yats" ["cajun"] 1)
         (Restaurant. "The Poki" ["poke"] -1)
         (Restaurant. "Soy Bistro" ["korean" "asian fusion"] 2)
         (Restaurant. "Corner Asian Bistro" ["chinese"] 2)
         (Restaurant. "Nama Sushi Bar" ["sushi"] 2)
         (Restaurant. "Umami" ["sushi" "korean" "thai"] 2)
         (Restaurant. "Chauhan Ale & Masala House" ["indian" "southern"] 2)
         (Restaurant. "Korea House" ["korean"] 2)
         (Restaurant. "Kayne Prime" ["steakhouse"] 4)
         (Restaurant. "The Smiling Elephant" ["thai"] 2)))

(defn first-restaurant [db]
  (:name (first db)))
...
;;
(defn welcome [pmsg]
  (clojure.string/join " " ["Welcome" (first (get pmsg :args))]))
...
(defn format-hour [h]
 (cond
   (= h 0) (clojure.string/join [12 "am"])
   (= h 12) (clojure.string/join [h "pm"])
   (< h 12) (clojure.string/join [h "am"])
   :else (clojure.string/join [(- h 12) "pm"])))
...
(defn formatted-hours [hours]
  (clojure.string/join " " ["from" (format-hour (get hours :start))
...
;; return output
;;
(defn action-send-msgs [[person & rest] msg]
  (if person
    (cons (action-send-msg person msg) (action-send-msgs rest msg))
    ()))
...
;;
(defn action-inserts [prefix [key & rest] v]
  (if key
    (cons (action-insert (conj prefix key) v) (action-inserts prefix rest v))
    ()))
...
(defn restaurant-register [db name cuisine price]
  (action-insert [:restaurants name] {:cuisine cuisine :price (read-string price)}))
...
(defn restaurant-unregister [db name]
  (action-remove [:restaurants name]))
...
(defn ask-experts [experts {:keys [args user-id]}]
 (if (empty? (rest args))
  ...
  (if (empty? experts)
   ...
   [(into [] (concat (action-inserts [:conversations] experts user-id) (action-send-msgs experts (clojure.string/join " " (rest args))))) (experts-question-msg experts (rest args))])))
...
(defn answer-question [conversation {:keys [args]}]
    (if (empty? args)
        ...
        (if (empty? conversation)
            ...
            [[(action-send-msg conversation (clojure.string/join " " args))] "Your answer was sent."])))
...
(defn add-restaurant [db {:keys [args user-id]}]
  [[(restaurant-register db (nth args 0) (nth args 1) (nth args 2))]
   (str (first args) " is now in the database.")])
...
(defn choose-restaurant [restaurants {:keys [args user-id]}]
  (if (empty? (rest args))
    [[] "You must choose a valid filter."]
        ...
        (if (= type "price")
          (let [res (clojure.string/join ", "
                                   (map (fn [res]
                                          (first res))
                                        (filter (fn [r]
                                                  (<= (:price (nth r 1)) (read-string target)))
                                                restaurants)))]
            [[(action-send-msg user-id res)] (str "Here are your option(s): " res ".")])
          (let [res (clojure.string/join ", "
                                   (map (fn [res]
                                          (first res))
                                        (filter (fn [r]
                                                  (= (:cuisine (nth r 1)) target))
                                                restaurants)))]
            [[(action-send-msg user-id res)] (str "Here are your option(s): " res ".")]))))))
...
(defn restaurants-in-db-query [state-mgr pmsg]
  (get! state-mgr [:restaurants]))
...
(def queries
  {"expert" experts-on-topic-query
   ...
   "choose" restaurants-in-db-query})
